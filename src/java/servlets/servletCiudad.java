 ...
 */
@WebServlet(name = "servletCiudad", urlPatterns = {"/ciudad"})
//http://localhost:8080/Multitareapps/ciudad?id_dep=
public class servletCiudad extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (!id_dep.equals("")) {
                Gson g = new Gson();
                LinkedList<Object> departamentos = ciudades(id_dep);
    ...
    public LinkedList<Object> ciudades(String id_dep) {
        LinkedList<Object> retorno = new LinkedList<>();
        DBConnect db = new DBConnect();
        ResultSet resl;
        String SQL = "SELECT id_ciudad, nombre\n"
                + "  FROM public.\"Ciudad\"\n"
                + "  WHERE fk_departamento = "+ id_dep+"";

        System.out.println(SQL);
        resl = db.executeQuery(SQL);
        try {
            while (resl.next()) {
                retorno.add(new Ciudad(resl.getInt(1), resl.getString(2)));
            }
